...
import { buildWhatsappChatUrl } from '@/lib/whatsapp-chat-url';
...
type Props = {
  /** Base wa.me / WhatsApp URL from admin_settings (passed from server layout). */
  whatsappUrl: string;
};

export default function WhatsAppButton({ whatsappUrl }: Props) {
  const [isTooltipVisible, setIsTooltipVisible] = useState(false);
  /** Only true after mount — FAB is not SSR’d, so server & first client pass both render null (no href/svg mismatch). */
  const [ready, setReady] = useState(false);
  const [chatUrl, setChatUrl] = useState('');

  useEffect(() => {
    const base = whatsappUrl?.trim() || SITE_CONFIG.whatsapp;
    setChatUrl(buildWhatsappChatUrl(base, DEFAULT_MESSAGE));
    setReady(true);
  }, [whatsappUrl]);

  const preventEmptyNav = useCallback(
    (e: React.MouseEvent<HTMLAnchorElement>) => {
      if (!ready || !chatUrl) e.preventDefault();
    },
    [ready, chatUrl],
  );
