...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: locale === 'nl' ? 'Conditions d\'utilisation' : 'Nutzungsbedingungen', alternates: { canonical: locale === 'nl' ? `${SITE_CONFIG.url}/terms` : `${SITE_CONFIG.url}/de/terms`, languages: { 'fr-CH': `${SITE_CONFIG.url}/terms`, 'de-CH': `${SITE_CONFIG.url}/de/terms`, 'x-default': `${SITE_CONFIG.url}/terms` } } };
}
...
export default async function TermsPage({ params }: Props) {
  ...
  const sections = isNl ? [
    ...
    { title: '3. Abonnement et paiement', content: 'Les prix sont indiqués en euros (EUR). L\'abonnement est activé dans les 2 heures suivant la confirmation du paiement.' },
    ...
    { title: '3. Abonnement und Zahlung', content: 'Die Preise sind in Euro (EUR) angegeben. Das Abonnement wird innerhalb von 2 Stunden nach Zahlungsbestätigung aktiviert.' },
  ...
  return (
    <div className="pt-28 pb-20 bg-white">
      <div className="max-w-2xl mx-auto px-5 sm:px-8">
        <h1 className="text-3xl font-extrabold text-text tracking-tight mb-8">{isNl ? 'Conditions d\'utilisation' : 'Nutzungsbedingungen'}</h1>
