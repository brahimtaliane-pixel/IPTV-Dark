-- Run in Supabase SQL Editor if /api/visitors/heartbeat fails (table missing).
-- Source: migrations/20260216100000_live_visitors.sql
...
DROP POLICY IF EXISTS "Service role full access live_visitors" ON live_visitors;
CREATE POLICY "Service role full access live_visitors" ON live_visitors FOR ALL USING (auth.role() = 'service_role');

DROP POLICY IF EXISTS "Anon can upsert live_visitors" ON live_visitors;
CREATE POLICY "Anon can upsert live_visitors" ON live_visitors FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Anon can update live_visitors" ON live_visitors;
